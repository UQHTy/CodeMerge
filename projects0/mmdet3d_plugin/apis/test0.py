...
#         #     # name 的形式通常是 "anchor_encoder.xxx" 或者 "sampler.xxx"，
#         #     # 你可以打印一下 det_head.named_parameters() 看一下具体前缀
#         #     if name.startswith("anchor_encoder") or name.startswith("sampler"):
#         #         continue
#         #     trainable_params.append(param)

#         det_params = list(model.module.head.det_head.parameters())
#         optimizer = optim.AdamW(det_params, lr=1e-6, weight_decay=1e-3) #5e-6,1e-6
...
#                 indices = None
#                 if len(RP)>5:
#                     # features_array = np.array(RP)
#                     # # features_tensor = torch.stack(RP, dim=0)
#                     # kmeans = KMeans(n_clusters=1, random_state=42)
#                     # kmeans.fit(features_array)
#                     # center = kmeans.cluster_centers_[0]
#                     # distances = np.linalg.norm(features_array - center, axis=1)
#                     # indices = np.argsort(distances)[-5:][::-1]
